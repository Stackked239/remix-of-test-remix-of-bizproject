...
function buildDeepDiveSection(idmOutput: LILIDMOutput): string {
  ...
  for (const code of PRIMARY_CATEGORIES) {
    ...
    for (const weakness of weaknesses) {
      ...
      findingsHtml.push(`<div class="finding-card" style="border-left-color: ${severity.borderColor};"><div class="finding-card-header"><h4>${weakness}</h4><span class="severity-badge" style="background: ${severity.bgColor}; color: ${severity.color};">${severity.label}</span></div><div class="finding-narrative"><p>This finding reflects a gap in ${catName} performance that directly affects the technology infrastructure and security posture of the business. Addressing it reduces operational risk and improves efficiency.</p></div><div class="finding-action"><strong>Impact Assessment:</strong> ${actionContext} See the Quick Wins and Action Plan sections for specific implementation steps.</div></div>`);
    }
...
export async function buildLilManagerItTechnologyReport(
  ...
  const report: LILGeneratedReport = {
    ...
    sections: ['Cover Page', 'Executive Summary', 'IT & Technology Health Snapshot', 'Priority Action', 'Priority Focus Areas', 'Gap to Excellence', 'Deep Dive: IT & Technology Dimensions', 'Quick Wins', 'IT & Technology Action Plan', 'Essential Tools & Templates', 'IT & Technology Metrics & 90-Day Targets', 'Progress Tracker', 'Next Steps'],
