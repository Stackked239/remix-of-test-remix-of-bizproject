/**
 * LIL Manager Operations Report Builder
 * 
 * Dedicated builder for the Manager Operations Report in the LIL (Essentials) pipeline.
 * v4.1: Role-based authority framing (no hat metaphors per exec team decision)
 * 
 * Primary categories: OPS, HRS
 * Secondary categories: CXP, FIN (for cross-functional context)
 * 
 * 7-Section Structure:
 * 1. Cover & Introduction + Company Context
 * 2. Operations Snapshot
 * 3. Key Metrics Dashboard
 * 4. Category Deep Dives (OPS, HRS)
...
const PRIMARY_CATEGORIES: CategoryCode[] = ['OPS', 'HRS'];
const ALL_SCOPED_CATEGORIES: CategoryCode[] = ['OPS', 'HRS', 'CXP', 'FIN'];
...
<head>
  ...
  <meta name="generator" content="BizHealth.ai LIL Manager Operations Report Builder">
  <meta name="robots" content="noindex, nofollow">
  <title>Manager Operations Report — {{COMPANY_NAME}}</title>
...
<body>
  <header class="report-header">
    ...
    <div class="header-meta">
      <div class="report-title">Manager Operations Report</div>
      <div>{{DATE}}</div>
    </div>
  </header>
  ...
  <footer class="report-footer">
    ...
    <p>Manager Operations Report — Essentials Plan</p>
...
function buildPriorityCallout(idmOutput: LILIDMOutput): string {
  ...
  if (!bestAction) { bestAction = 'Review the operations and HR dimension scores below and select the highest-priority area for immediate action.'; bestCategory = 'Operations'; }
  return `<div class="priority-callout"><h3>Priority Action</h3><p class="callout-action">${bestAction}</p><p class="callout-meta">⏱ Estimated effort: 30–60 minutes | ⚙️ Directly impacts ${bestCategory} efficiency</p></div>`;
}
...
function buildGapChart(idmOutput: LILIDMOutput): string {
  ...
  return `<div class="gap-chart-section"><h2>Gap to Excellence</h2><p>Distance from the Excellence threshold (80/100) by operations dimension.</p>${barsHtml}</div>`;
}
...
function buildDeepDiveSection(idmOutput: LILIDMOutput): string {
  ...
  return `<div class="deep-dive-section"><h2>Deep Dive: Operations Dimensions</h2><p>The following analysis examines each operations dimension, highlighting current performance and identifying specific opportunities for improvement.</p>${findingsHtml.join('')}</div>`;
}
...
function buildQuickWinsSection(idmOutput: LILIDMOutput): string {
  ...
  for (const code of ALL_SCOPED_CATEGORIES) {
    ...
    for (const rec of recs) {
      ...
      quickWinsHtml.push(`<div class="quick-win-card"><div class="quick-win-header"><h4>${title}</h4><div class="quick-win-score-change"><span class="arrow-up">▲</span> ${catScore} → ${outcome.expectedScore}</div></div><div class="quick-win-body"><p>${safeStr(rec.description, 'Targeted improvement in this area will yield measurable results.')}</p>${rec.estimatedImpact ? `<p><strong>Expected Impact:</strong> ${rec.estimatedImpact}</p>` : ''}</div><div class="quick-win-meta"><span>📂 ${catName}</span><span>⏱ ${safeStr(rec.timeframe, '30-day')}</span><span>📈 Closing ${outcome.gapClosurePercent}% of gap to benchmark</span><span class="accountable-tag">👤 Operations Lead</span></div></div>`);
    }
  }
  return `<div class="quick-wins-section"><h2>Quick Wins: Highest-Impact Actions</h2><p>The following actions target specific gaps in operations and HR scores and can be implemented within 30 days for measurable improvement.</p>${quickWinsHtml.join('')}</div>`;
}
...
function buildActionPlanSection(idmOutput: LILIDMOutput): string {
  ...
  return `<div class="action-plan-section"><h2>Operations Action Plan</h2><p>A phased implementation plan organized by priority. Each phase builds on the previous phase's progress.</p><div class="action-plan-month month-1"><h3>📋 Phase 1: Days 1–30</h3>${buildActionItems(thirtyDayItems)}</div><div class="action-plan-month month-2"><h3>📋 Phase 2: Days 31–60</h3>${buildActionItems(sixtyDayItems)}</div><div class="action-plan-month month-3"><h3>📋 Phase 3: Days 61–90</h3>${buildActionItems(ninetyDayItems)}</div>${habitsHtml}</div>`;
}

function buildToolsSection(): string {
  return `<div class="tools-section"><h2>Essential Tools & Templates</h2><p>Recommended resources to support operations and HR improvement initiatives.</p>
    <div class="tool-card"><div class="tool-icon">⚙️</div><div><h4>Process Mapping Template</h4><p>Document and visualize core workflows to identify bottlenecks, redundancies, and automation opportunities. Use quarterly to maintain operational efficiency.</p></div></div>
    <div class="tool-card"><div class="tool-icon">👥</div><div><h4>Employee Engagement Survey</h4><p>Measure workforce satisfaction, identify retention risks, and track improvement over time. Includes benchmarking against industry standards.</p></div></div>
    <div class="tool-card"><div class="tool-icon">📋</div><div><h4>Standard Operating Procedures (SOP) Builder</h4><p>Create, maintain, and distribute SOPs for consistent execution across teams. Includes version control and acknowledgment tracking.</p></div></div>
    <div class="tool-card"><div class="tool-icon">📊</div><div><h4>Capacity Planning Worksheet</h4><p>Forecast staffing needs, track utilization rates, and plan for seasonal demand fluctuations. Essential for scaling operations efficiently.</p></div></div></div>`;
}
...
function buildMetricsSection(idmOutput: LILIDMOutput): string {
  ...
  return `<div class="metrics-section"><h2>Operations Metrics & 90-Day Targets</h2><p>Current performance by dimension with projected targets based on recommended actions.</p>${metricsHtml.join('')}</div>`;
}
...
async function generateNarrativeSummary(idmOutput: LILIDMOutput, businessOverview: LILBusinessOverview): Promise<{ narrative: string; tokensUsed: number }> {
  ...
  const prompt = `You are BizHealth.ai's operations analyst writing a professional executive summary for ${companyName}'s operations report.
...
- Treat the reader as a competent decision-maker responsible for operations and HR

Write a 3-4 paragraph executive summary (150-200 words total) that:
1. Opens with ${companyName}'s strongest operations dimension and what it means
...
4. Closes with a forward-looking statement about operational efficiency trajectory
...
export async function buildLilManagerOperationsReport(
