/**
 * LIL Manager Sales & Marketing Report Builder
 * 
 * Dedicated builder for the Manager Sales & Marketing Report in the LIL (Essentials) pipeline.
 * v4.1: Role-based authority framing (no hat metaphors per exec team decision)
 * 
 * Primary categories: SAL, MKT, CXP
 * Secondary categories: STR (for strategic context)
 * 
 * 7-Section Structure:
 * 1. Cover & Introduction + Company Context
 * 2. Sales & Marketing Snapshot
 * 3. Key Metrics Dashboard
 * 4. Category Deep Dives (SAL, MKT, CXP)
...
const PRIMARY_CATEGORIES: CategoryCode[] = ['SAL', 'MKT', 'CXP'];
const ALL_SCOPED_CATEGORIES: CategoryCode[] = ['SAL', 'MKT', 'CXP', 'STR'];
...
function calculateExpectedOutcome(currentScore: number, benchmarkScore: number = 80): { expectedScore: number; improvementPoints: number; gapClosurePercent: number } {
  ...
  const expectedScore = currentScore + improvementPoints;
...
<head>
  ...
  <meta name="generator" content="BizHealth.ai LIL Manager Sales & Marketing Report Builder">
  <meta name="robots" content="noindex, nofollow">
  <title>Manager Sales & Marketing Report — {{COMPANY_NAME}}</title>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .report-header {
      ...
      align-items: center;
      padding: 20px 40px;
      border-bottom: 4px solid var(--biz-blue);
      background: linear-gradient(135deg, #f8f9fa 0%, #ffffff 100%);
    }

    .header-logo { font-family: 'Montserrat', sans-serif; font-size: 1.5rem; font-weight: 700; color: var(--biz-blue); }
    .header-logo span { color: var(--biz-green); }
    .header-meta { text-align: right; font-size: 0.875rem; color: var(--text-secondary); }
    .header-meta .report-title { font-family: 'Montserrat', sans-serif; font-weight: 600; color: var(--biz-blue); }

    /* Cover Page */
    .cover-page {
      min-height: 100vh;
      background: linear-gradient(135deg, var(--biz-blue) 0%, #1a1b3d 50%, #2d3570 100%);
      color: white;
      display: flex;
    ...
    .priority-callout h3 { color: var(--biz-green); font-size: 1.1rem; margin-bottom: 0.75rem; }
    .priority-callout .callout-action { font-size: 1.125rem; line-height: 1.6; margin-bottom: 0.5rem; }
    .priority-callout .callout-meta { font-size: 0.875rem; opacity: 0.8; }
    ...
    .overall-score-circle .score-number { font-family: 'Montserrat', sans-serif; font-size: 3rem; font-weight: 700; line-height: 1; }
    .overall-score-circle .score-label { font-size: 0.8rem; font-weight: 600; text-transform: uppercase; letter-spacing: 1px; margin-top: 4px; }
    ...
    .dimension-bar-label { font-size: 0.9rem; font-weight: 500; color: var(--text-primary); }
    .dimension-bar-track { height: 24px; background: #e9ecef; border-radius: 12px; overflow: hidden; position: relative; }
    .dimension-bar-fill { height: 100%; border-radius: 12px; }
    .dimension-bar-benchmark { position: absolute; top: 0; height: 100%; width: 3px; background: var(--biz-blue); opacity: 0.6; }
    .dimension-bar-score { font-family: 'Montserrat', sans-serif; font-size: 1rem; font-weight: 700; text-align: right; }
    ...
    .priority-table td { padding: 14px 16px; border-bottom: 1px solid var(--border-color); font-size: 0.95rem; }
    ...
    .evidence-citation { background: #f8f9fa; border-left: 4px solid var(--biz-green); padding: 0.75rem 1rem; margin: 0.75rem 0; font-size: 0.9rem; border-radius: 0 6px 6px 0; }
    .evidence-citation strong { color: var(--biz-blue); }
    ...
    .quick-win-header { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 1rem; }
    .quick-win-header h4 { margin: 0; flex: 1; }
    .quick-win-score-change { display: flex; align-items: center; gap: 6px; font-family: 'Montserrat', sans-serif; font-size: 0.9rem; font-weight: 600; white-space: nowrap; }
    .quick-win-score-change .arrow-up { color: var(--excellence); }
    .quick-win-body { font-size: 0.95rem; line-height: 1.7; }
    .quick-win-meta { display: flex; gap: 1.5rem; margin-top: 1rem; padding-top: 0.75rem; border-top: 1px solid var(--border-color); font-size: 0.85rem; color: var(--text-secondary); flex-wrap: wrap; }
    ...
    .action-item { display: grid; grid-template-columns: 24px 1fr; gap: 10px 12px; align-items: start; padding: 14px 0; border-bottom: 1px solid #eee; font-size: 1rem; line-height: 1.5; }
    ...
    .action-time { font-size: 0.85rem; color: var(--text-secondary); background: #f5f5f5; padding: 2px 10px; border-radius: 12px; font-weight: 500; }
    .action-impact { font-size: 0.85rem; color: var(--proficiency); font-weight: 600; word-wrap: break-word; overflow-wrap: break-word; }
    ...
    .ongoing-habits h3 { display: flex; align-items: center; gap: 8px; }
    .habit-item { padding: 8px 0; font-size: 0.95rem; display: flex; align-items: center; gap: 8px; }
    ...
    .metric-enhanced-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.75rem; }
    .metric-enhanced-header h4 { margin: 0; font-size: 0.95rem; }
    .metric-scores { display: flex; gap: 1rem; align-items: baseline; }
    .metric-current { font-family: 'Montserrat', sans-serif; font-size: 1.5rem; font-weight: 700; }
    .metric-target { font-size: 0.85rem; color: var(--text-secondary); }
    .metric-target span { font-weight: 600; color: var(--proficiency); }
    .metric-progress-bar { height: 8px; background: #e9ecef; border-radius: 4px; overflow: hidden; margin: 0.5rem 0; }
    .metric-progress-fill { height: 100%; border-radius: 4px; }
    ...
    .gap-bar-container { height: 28px; background: #e9ecef; border-radius: 6px; overflow: hidden; position: relative; }
    .gap-bar-current { height: 100%; border-radius: 6px; position: relative; }
    .gap-bar-target { position: absolute; top: 0; height: 100%; width: 3px; background: var(--biz-blue); z-index: 2; }
    .gap-bar-remaining { position: absolute; top: 0; height: 100%; background: repeating-linear-gradient(45deg, transparent, transparent 4px, rgba(0,0,0,0.05) 4px, rgba(0,0,0,0.05) 8px); border-radius: 0 6px 6px 0; }
    .gap-bar-value { font-family: 'Montserrat', sans-serif; font-size: 0.9rem; font-weight: 600; text-align: right; }

    .tools-section { margin: 2.5rem 0; }
    .tool-card { padding: 1.25rem; margin: 1rem 0; background: white; border: 1px solid var(--border-color); border-radius: 10px; box-shadow: var(--card-shadow); display: grid; grid-template-columns: auto 1fr; gap: 1rem; align-items: start; }
    .tool-icon { font-size: 2rem; }
    .tool-card h4 { margin: 0 0 0.5rem 0; }
    .tool-card p { margin: 0; font-size: 0.9rem; color: var(--text-secondary); }

    .progress-tracker { margin: 2.5rem 0; }
    .tracker-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 1rem; margin: 1.5rem 0; }
    .tracker-card { padding: 1.25rem; background: white; border: 1px solid var(--border-color); border-radius: 10px; box-shadow: var(--card-shadow); text-align: center; }
    .tracker-card .tracker-score { font-family: 'Montserrat', sans-serif; font-size: 2rem; font-weight: 700; }
    .tracker-card .tracker-label { font-size: 0.85rem; color: var(--text-secondary); margin-top: 0.25rem; }
    .tracker-card .tracker-target { font-size: 0.8rem; color: var(--proficiency); font-weight: 600; margin-top: 0.5rem; }

    .report-footer {
      margin-top: 3rem; padding: 2rem 40px;
      background: var(--biz-blue); color: white; text-align: center;
    }
    .footer-logo { font-family: 'Montserrat', sans-serif; font-size: 1.25rem; font-weight: 700; margin-bottom: 0.5rem; }
    .footer-logo span { color: var(--biz-green); }
    .report-footer p { font-size: 0.85rem; opacity: 0.8; margin-bottom: 0.25rem; }

    @media print {
      body { font-size: 11pt; }
      .report-container { padding: 0 20px; }
      .cover-page { min-height: 100vh; page-break-after: always; }
      .priority-callout { break-inside: avoid; }
...
<body>
  <header class="report-header">
    ...
    <div class="header-meta">
      <div class="report-title">Manager Sales & Marketing Report</div>
      <div>{{DATE}}</div>
    </div>
  </header>
  ...
  <footer class="report-footer">
    ...
    <p>Manager Sales & Marketing Report — Essentials Plan</p>
...
function buildCoverPage(companyName: string, currentDate: string): string {
  return `
    <div class="cover-page">
      ...
      <h1>Manager Sales & Marketing Report</h1>
      <div class="company-name">${companyName}</div>
      <div class="cover-divider"></div>
      <div class="role-framing">Revenue generation and market presence are the growth engines of ${companyName}. This report analyzes sales performance, marketing effectiveness, and customer experience to deliver actionable recommendations.</div>
...
function buildSnapshotSection(idmOutput: LILIDMOutput): string {
  ...
  return `
    <div class="snapshot-section">
      <h2>Sales & Marketing Health Snapshot</h2>
...
function buildPriorityCallout(idmOutput: LILIDMOutput): string {
  ...
  if (!bestAction) {
    bestAction = 'Review the sales and marketing dimension scores below and select the highest-priority area for immediate action.';
    bestCategory = 'Sales & Marketing';
  }
...
function buildPriorityFocusTable(idmOutput: LILIDMOutput): string {
    ...
    .map(dim => {
      ...
      return `
        <tr>
          ...
          <td style="text-align: center;"><span style="font-family: 'Montserrat', sans-serif; font-weight: 700; color: ${getScoreBand(dim.score).color};">${dim.score}</span></td>
          <td style="text-align: center;"><span style="color: ${gapColor}; font-weight: 600;">${gapStr}</span></td>
          <td style="text-align: center;"><span class="priority-badge" style="background: ${tier.bgColor}; color: ${tier.color};">${tier.label}</span></td>
  ...
  return `
    <div class="snapshot-section">
      ...
      <p>Sales and marketing dimensions ranked by urgency, with gap-to-benchmark analysis and recommended focus level.</p>
      <table class="priority-table">
        <thead>
          <tr><th>Dimension</th><th style="text-align: center;">Score</th><th style="text-align: center;">vs Benchmark</th><th style="text-align: center;">Priority</th><th>Recommendation</th></tr>
        </thead>
...
function buildGapChart(idmOutput: LILIDMOutput): string {
  ...
  const barsHtml = PRIMARY_CATEGORIES.map(code => {
    ...
    return `
      <div class="gap-bar-row">
        ...
        <div class="gap-bar-value" style="color: ${band.color};">${gap > 0 ? `−${gap} gap` : '✓ On track'}</div>
      </div>`;
  }).join('');
  ...
  return `
    <div class="gap-chart-section">
      ...
      <p>Distance from the Excellence threshold (80/100) by sales and marketing dimension.</p>
      ${barsHtml}
    </div>`;
}
...
function buildDeepDiveSection(idmOutput: LILIDMOutput): string {
      ...
      findingsHtml.push(`
        <div class="finding-card" style="border-left-color: #28a745;">
          ...
          <div class="finding-narrative">
            <ul style="margin-left: 18px;">${strengths.map(s => `<li>${s}</li>`).join('')}</ul>
          </div>
        </div>`);
    }
    ...
    for (const weakness of weaknesses) {
      ...
      findingsHtml.push(`
        <div class="finding-card" style="border-left-color: ${severity.borderColor};">
          ...
          <div class="finding-narrative">
            <p>This finding reflects a gap in ${catName} performance that directly affects revenue generation and market positioning. Addressing it strengthens the growth engine of the business.</p>
          </div>
  ...
  return `
    <div class="deep-dive-section">
      <h2>Deep Dive: Sales & Marketing Dimensions</h2>
      <p>The following analysis examines each sales and marketing dimension, highlighting current performance and identifying specific opportunities for improvement.</p>
      ${findingsHtml.join('')}
    </div>`;
}
...
function buildQuickWinsSection(idmOutput: LILIDMOutput): string {
        ...
        <div class="quick-win-card">
          <div class="quick-win-header">
            ...
            <div class="quick-win-score-change">
              <span class="arrow-up">▲</span> ${catScore} → ${outcome.expectedScore}
            </div>
          </div>
          ...
          <div class="quick-win-meta">
            ...
            <span class="accountable-tag">👤 Sales & Marketing Lead</span>
          </div>
        </div>`);
    }
  }
  ...
  return `
    <div class="quick-wins-section">
      ...
      <p>The following actions target specific gaps in sales and marketing scores and can be implemented within 30 days for measurable improvement.</p>
      ${quickWinsHtml.join('')}
    </div>`;
}
...
function buildActionPlanSection(idmOutput: LILIDMOutput): string {
  ...
  // If any phase is still empty, generate contextual actions from category weaknesses and quick wins
  if (thirtyDayItems.length === 0 || sixtyDayItems.length === 0 || ninetyDayItems.length === 0) {
    ...
    if (ninetyDayItems.length === 0 && generatedActions.length > 6) {
      ninetyDayItems = generatedActions.slice(6, 9);
    } else if (ninetyDayItems.length === 0 && generatedActions.length > 0) {
      ninetyDayItems = generatedActions.slice(0, 3);
    }
  }
  ...
  return `
    <div class="action-plan-section">
      <h2>Sales & Marketing Action Plan</h2>
...
function buildToolsSection(): string {
  return `
    <div class="tools-section">
      <h2>Essential Tools & Templates</h2>
      <p>Recommended resources to support sales and marketing improvement initiatives.</p>
      <div class="tool-card">
        <div class="tool-icon">📊</div>
        <div>
          <h4>Sales Pipeline Tracker</h4>
          <p>Track leads, conversion rates, and deal stages to identify bottlenecks in the sales process. Use weekly to monitor pipeline health.</p>
        </div>
      </div>
      <div class="tool-card">
        <div class="tool-icon">📋</div>
        <div>
          <h4>Marketing Campaign Planner</h4>
          <p>Plan, execute, and measure marketing campaigns with clear KPIs. Includes budget tracking and ROI calculation templates.</p>
        </div>
      </div>
      <div class="tool-card">
        <div class="tool-icon">🎯</div>
        <div>
          ...
          <p>Visualize the customer experience from first contact to retention. Identify pain points and opportunities to improve satisfaction.</p>
        </div>
      </div>
      <div class="tool-card">
        <div class="tool-icon">📈</div>
        <div>
          ...
          <p>Systematically evaluate competitors' strengths and weaknesses to inform positioning and differentiation strategies.</p>
        </div>
      </div>
    </div>`;
}
...
function buildMetricsSection(idmOutput: LILIDMOutput): string {
  ...
  for (const code of PRIMARY_CATEGORIES) {
    ...
    const catName = CATEGORY_NAMES[code] || code;
    const catScore = safeNum(catData.score, 0);
    const targetScore = Math.min(catScore + 15, 100);
    ...
    metricsHtml.push(`
      <div class="metric-enhanced-card">
        ...
        <div class="metric-guidance">
          <strong>What moves this score:</strong> Implementing the Quick Wins above for ${catName.toLowerCase()} directly impacts this metric. Target: ${targetScore}/100 within 90 days.
        </div>
      </div>`);
  }
  ...
  return `
    <div class="metrics-section">
      <h2>Sales & Marketing Metrics & 90-Day Targets</h2>
...
function buildProgressTracker(idmOutput: LILIDMOutput): string {
  const trackerCards = PRIMARY_CATEGORIES.map(code => {
    const score = safeNum(idmOutput.healthScores?.byCategory?.[code], 0);
    const band = getScoreBand(score);
    const targetScore = Math.min(score + 15, 100);
    const name = CATEGORY_NAMES[code] || code;
    return `
      <div class="tracker-card">
        <div class="tracker-score" style="color: ${band.color};">${score}</div>
        <div class="tracker-label">${name}</div>
        <div class="tracker-target">90-day target: ${targetScore}</div>
      </div>`;
  }).join('');
  ...
  return `
    <div class="progress-tracker">
      ...
      <p>Use this section to track progress against 90-day targets. Re-assess quarterly to measure improvement.</p>
      <div class="tracker-grid">
        ${trackerCards}
      </div>
      <div class="evidence-citation" style="margin-top: 1.5rem;">
        <strong>📅 Next Assessment:</strong> Schedule a follow-up assessment in 90 days to measure progress against these targets and recalibrate priorities.
      </div>
    </div>`;
}
...
function buildClosingSection(
  ...
  const callToAction = bluf?.callToAction || 'Schedule a follow-up consultation to discuss sales and marketing priorities.';
  const overallScore = safeNum(idmOutput.healthScores?.overall, 0);
  const band = getScoreBand(overallScore);
  
  return `
    <div class="priority-callout" style="margin-top: 3rem;">
      <h3>Next Steps</h3>
      <p class="callout-action">${callToAction}</p>
      <p class="callout-meta">
        Current overall health: <strong>${overallScore}/100 (${band.label})</strong>. 
        Executing the action plan above positions ${companyName} for measurable revenue growth within 90 days.
      </p>
    </div>`;
}
...
async function generateNarrativeSummary(
  ...
  const prompt = `You are BizHealth.ai's sales and marketing analyst writing a professional executive summary for ${companyName}'s sales and marketing report.
...
- Treat the reader as a competent decision-maker responsible for sales and marketing

Write a 3-4 paragraph executive summary (150-200 words total) that:
1. Opens with ${companyName}'s strongest sales/marketing dimension and what it means
...
4. Closes with a forward-looking statement about revenue growth trajectory
...
Output ONLY the HTML paragraphs (using <p> tags). No headings, no wrapper divs.`;
  ...
  try {
    ...
    return {
      narrative: `<p>The sales and marketing assessment reveals a current health score of ${overallScore}/100, indicating a foundation with clear opportunities for revenue growth. The priority areas identified below represent the highest-impact focus points for the next 90 days.</p>`,
      tokensUsed: 0
    };
  }
}
...
export async function buildLilManagerSalesMarketingReport(
  idmOutput: LILIDMOutput,
  businessOverview: LILBusinessOverview,
  bluf?: LILPhase4_5Output['blufs'][keyof LILPhase4_5Output['blufs']]
  ...
  logger.info({ companyName }, 'Building LIL Manager Sales & Marketing Report (v4.1)');
  ...
  const report: LILGeneratedReport = {
    reportType: 'manager-sales-marketing' as any,
    title: "Manager Sales & Marketing Report",
    htmlContent,
    pageCount,
    sections: [
      'Cover Page',
      'Executive Summary',
      'Sales & Marketing Health Snapshot',
      'Priority Action',
      'Priority Focus Areas',
      'Gap to Excellence',
      'Deep Dive: Sales & Marketing Dimensions',
      'Quick Wins',
      'Sales & Marketing Action Plan',
      'Essential Tools & Templates',
      'Sales & Marketing Metrics & 90-Day Targets',
      'Progress Tracker',
      'Next Steps'
    ],
  ...
  logger.info({ pageCount, tokensUsed, sectionsGenerated: report.sections.length, htmlLength: htmlContent.length }, 'LIL Manager Sales & Marketing Report (v4.1) built successfully');
