...
import { Rocket, Lightbulb, Square, CheckSquare } from "lucide-react";
import { Checkbox } from "@/components/ui/checkbox";

interface Module5Day4Props {
  isComplete: boolean;
...
const ACTION_ITEMS = [
  {
    title: "Gather your customer list (email addresses of 10-20 recent buyers/users)",
    subItems: [
  ...
  { title: "Open your email tool (Gmail, Outlook, Mailchimp, etc.)" },
  { title: "Paste the survey email you wrote on Day 2" },
  { title: "Insert the customer names (use mail merge if your tool supports it)" },
  { title: "Send it today (or schedule for tomorrow morning)" },
  { title: "Open your feedback tracker—add a note that you've sent the survey" }
];

const Module5Day4 = ({ isComplete, onToggleComplete }: Module5Day4Props) => {
  const ref = useRef(null);
  const isInView = useInView(ref, { once: true, margin: "-100px" });

  return (
    <section ref={ref} className="py-12 md:py-16 bg-muted/30 scroll-mt-48" id="day-4">
          ...
          <div className="flex items-center justify-between p-6 border-b bg-gradient-to-r from-[hsl(var(--biz-green))]/15 to-[hsl(var(--biz-green))]/5">
            <div className="flex items-center gap-4">
              <div className={`w-12 h-12 rounded-xl flex items-center justify-center
                ...
                <Rocket className={`w-6 h-6 ${isComplete ? 'text-[hsl(var(--biz-green))]' : 'text-[hsl(var(--biz-navy))]'}`} />
              </div>
              <div>
                <div className="flex items-center gap-2">
                  <span className="text-sm font-bold text-[hsl(var(--biz-navy))] uppercase tracking-wide">Day 4</span>
                  <span className="text-sm text-muted-foreground">⏱ 15 minutes</span>
                </div>
                <h2 className="text-xl md:text-2xl font-bold text-foreground">Launch Your First Survey</h2>
              </div>
            </div>
            <button
              ...
              <div className="space-y-4">
                {ACTION_ITEMS.map((item, index) => (
                  <div key={index} className="flex items-start gap-3">
                    <Checkbox id={`day4-action-${index}`} className="mt-1" />
                    <div className="flex-1">
                      <label 
                        htmlFor={`day4-action-${index}`}
